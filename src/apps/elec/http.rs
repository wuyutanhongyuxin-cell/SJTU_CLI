//! 电费 HTTP Client 构造 + 公共 JSON GET 封装。
//!
//! 与 `apps/services/http.rs` 同构（都是 my.sjtu 域族下的 REST API），独立一份避免
//! 跨子系统耦合（CLAUDE.md 项目专属约束）。BASE 切到 `elec.sjtu.edu.cn`，Referer
//! 切到 `/ui/recharge`（§7.1）。
//!
//! 必备 header（§7.1）：
//! - `X-Requested-With: XMLHttpRequest`（缺则可能被 HTML 兜底）
...
pub(super) const BASE: &str = "https://elec.sjtu.edu.cn";

/// 注入 `*.sjtu.edu.cn` 域 cookie 的 reqwest Client。
pub(super) fn build_http_client(session: &Session) -> Result<Client> {
    let jar = Arc::new(Jar::default());
    let elec_url: Url = "https://elec.sjtu.edu.cn/".parse().expect("const URL");
...
async fn fetch_once<T: serde::de::DeserializeOwned>(
    ...
    let resp = http
        ...
        .header(REFERER, format!("{BASE}/ui/recharge"))
