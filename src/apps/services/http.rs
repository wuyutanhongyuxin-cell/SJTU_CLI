//! 办事大厅 HTTP Client 构造 + 公共 JSON GET 封装。
//!
//! 与 `apps/jwbmessage/http.rs` 同构（同后端 my.sjtu.edu.cn），独立一份
//! 避免跨模块耦合（CLAUDE.md 项目专属约束："多子系统适配，每个子系统独立"）。
//!
//! 请求头硬约束（§6.1）：
//! - `X-Requested-With: XMLHttpRequest`（缺则被路由到 HTML 兜底）
...
async fn fetch_once<T: serde::de::DeserializeOwned>(
    ...
    let resp = http
        ...
        .header(REFERER, format!("{BASE}/ui/task/todo"))
